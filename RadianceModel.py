...
def binning(x, y, xbin):
    ...
    for i_bin in range(xbin.size):
        ...
        if indices_within_bin[0] != 0:
            leftoverlap = (x[indices_within_bin[0]] - binstart) / (x[indices_within_bin[0]] - x[indices_within_bin[0]-1])      
            binvalue = binvalue + leftoverlap * y[indices_within_bin[0]-1]
        else:
            leftoverlap = 0
    
        if indices_within_bin[-1] != x.size-1:   
            rightoverlap = (binend - x[indices_within_bin[-1]]) / (x[indices_within_bin[-1]+1] - x[indices_within_bin[-1]])
...
class RadianceModel():
    ...
    def getRadiance(self, nCH4, albedo, sza = 10, vza = 0):
        ...
        self.nCH4= nCH4*np.ones(24)
        self.nCH4 = np.reshape(self.nCH4,(self.data_abs[:,3].shape[0],1))   
