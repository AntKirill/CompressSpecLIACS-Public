...
instrumentsettings.aperture_act = 50e-3
instrumentsettings.aperture_alt = 50e-3
...
#%%
n = 500
nCH4_range = np.linspace(100,2000,20)
...
for j, config in enumerate(snrconfig):
    _, sza, albedo = config
    for i, nCH4 in enumerate(nCH4_range):
        # retrieve methane
        relative_fitprecision, relative_fitbias,  = instrument.simulateMeasurement(
            selection, nCH4=nCH4, albedo=albedo, sza = sza, n=n, verbose=False)
        
        nCH4_fit[j,i] = relative_fitbias * nCH4 + nCH4
        nCH4_std[j,i] = relative_fitprecision * nCH4
        
#%%
  
fig = plt.figure(dpi = 300, figsize = (4.5,3.7))
j = 1; config = snrconfig[j]; plt.errorbar(nCH4_range, nCH4_fit[j,:], nCH4_std[j,:], label = f'{config[0]} SNR, sza: {config[1]}, albedo: {config[2]}')
j = 0; config = snrconfig[j]; plt.errorbar(nCH4_range, nCH4_fit[j,:], nCH4_std[j,:], label = f'{config[0]} SNR, sza: {config[1]}, albedo: {config[2]}')
