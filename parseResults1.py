...
def main(dir):
    ...
    with open(os.path.join(dir, 'processed.csv'), 'w') as file:
        ...
        print('experiment_root', 'iteration', 'mean', 'std', 'mean_best_so_far', 'std_best_so_far', 'cnt', 'min', *argmin_components, file=file)
        for iteration in range(1, M + 1):
            ...
            for j in instances:
                instance_cur_value = lc.loc[lc['instance'] == j]['raw_y'].to_numpy(dtype=float)[0]
                best_so_far[iteration][j] = min(best_so_far[iteration - 1][j], instance_cur_value)
            ...
            print(dir, iteration, np.mean(vals), np.std(vals), mean_best_so_far, std_best_so_far, len(vals), np.min(vals), *best, file=file)
