...
class InstrumentSimulator():
    ...
    projected_orbital_speed = orbital_speed * earthradius / (altitude + earthradius)    # orbital speed projected on earth surface
    ...
    def __init__(self,instrumentsettings):
        ...
        self.giFOV = self.instrumentsettings.resolution / self.instrumentsettings.oversampling                       # Ground-projected instantaneous field-of-view
        self.swath_act = self.giFOV * self.instrumentsettings.detector.npxl_act                         # swath ACT in m
        self.swath_alt = self.giFOV * self.instrumentsettings.detector.npxl_alt                          # swath ALT in m

        self.focallength =  self.altitude / self.giFOV * self.instrumentsettings.detector.pxlsize             # in m
        ...
        self.etendue = self.instrumentsettings.aperture_alt * self.instrumentsettings.aperture_act * self.giFOV**2 / self.altitude**2  # in sr m2

        self.integrationtime = self.giFOV / self.projected_orbital_speed                              # in seconds
        self.readoutfreq = 1 / self.integrationtime                                            # in Hz
    ...
    def loadFilterLibrary(self):
        shapes = ['circle','cross','cross-hw','octagon','square','square-bcc']
        ...
        for shape in shapes:
            filename = os.path.join(module_directory,f'FilterLibrary/FilterLibrary_{shape}_11-08-2022.txt')
                 ...
                 for rowindex, row in enumerate(reader):
                     if rowindex == 0:
                         ...
                         if np.sum((np.float_(row[2:])) < 1.1) == len(wavelength) and np.sum((np.float_(row[2:])) > -0.0 ) == len(wavelength) :
                             transmissionprofiles = np.vstack((transmissionprofiles,np.float_(row[2:])))
                         else:
                             transmissionprofiles = np.vstack((transmissionprofiles,np.zeros((1,len(wavelength)))))
                             
            if self.filterlibrary.size == 0:
                self.filterlibrary = transmissionprofiles
            else:
                self.filterlibrary = np.vstack((self.filterlibrary, transmissionprofiles))
               
            
        # remove wavelengths larger than 1670 nm
        ind = self.wavelength_min <= wavelength * 1e9
        self.ind = np.logical_and(ind, wavelength * 1e9 < self.wavelength_max)
        self.filterlibrary = np.delete(self.filterlibrary, ~self.ind, axis = 1)
    ...
    def loadRadianceModel(self):
        ...
        self.radiancemodel = RadianceModel(lambda_min = 1625, 
                                           lambda_max = 1670, 
                                           lambda_n = 225,
                                           sza = self.instrumentsettings.sza,
    ...
    def filterguess(self):
        ...
        for i in range(len(self.structlabels)):
            p, residuals, rank, singular_values, rcond = np.polyfit(spectral_range,self.filterlibrary[i,:],5, full = True)
            smoothness[i] = residuals  
            
        nfilters = self.instrumentsettings.detector.npxl_alt
    ...
    def simulateMeasurement(self, selection, nCH4 = 1500, albedo = 0.15, noise = True, n = 100):
        ...
        if noise:
            ...
            relative_fitbias = np.abs(nCH4 - np.nanmean(nCH4_fit))  / nCH4
            
        else:
            nCH4_fit = self._fitMethane(self.signal)
            relative_fitbias = np.abs(nCH4 - nCH4_fit)  / nCH4
            relative_fitprecision = None
    ...
    def _fitMethane(self, signal_noisy):
        p0 = [1000, 0.15]
        fit = optimize.least_squares(self._getFiterror, p0, args = ([], signal_noisy))
